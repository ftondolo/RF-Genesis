...
# Depth Pointcloud Visualization
def draw_depth_pointcloud(depth_pc, ax, elev=30, azim=30):
    """
    Visualizes high-resolution depth pointcloud from dedicated depth sensor.
    
    Args:
        depth_pc: numpy array of shape (N, 3) with filtered 3D points
    ...
    # Plot with higher density for better visualization
...
def draw_combined(i, pointcloud_cfg, radar_frames, radar_pointclouds, depth_pointclouds):
    """
    Combine depth pointcloud, radar pointcloud, and Doppler FFT visualizations.
    
    Args:
        i: Frame index (30 FPS for video)
        pointcloud_cfg: Radar pointcloud configuration
        radar_frames: Raw radar frames
        radar_pointclouds: Processed radar pointclouds (from radar pipeline)
        depth_pointclouds: High-res depth pointclouds (from depth sensor)
    """
    radar_frame_id = int(i/3)  # Convert 30 FPS video to 10 FPS radar

    fig = plt.figure(figsize=(12, 6))

    # Panel 1: High-resolution depth pointcloud from dedicated sensor
    ax1 = fig.add_subplot(131, projection='3d')
    draw_depth_pointcloud(depth_pointclouds[radar_frame_id], ax1, elev=30, azim=30)

    # Panel 2: Radar pointcloud
...
def save_video(radar_cfg_file, radar_frames_file, depth_pointclouds_file, output_file):
    """
    Create visualization video with depth pointclouds, radar pointclouds, and Doppler FFT.
    
    Args:
        radar_cfg_file: Path to radar configuration file
        radar_frames_file: Path to radar frames .npy file
        depth_pointclouds_file: Path to depth pointclouds .npy file (from pathtracer)
    ...
    # Depth pointclouds are at 10 FPS, video is at 30 FPS
    num_depth_frames = len(depth_pointclouds)
    num_video_frames = num_depth_frames * 3  # 3x for 30 FPS
    
    # Write the video
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    out = cv2.VideoWriter(output_file, fourcc, 30, (1200, 600))
    
    for i in tqdm(range(min(num_video_frames - 2, len(depth_pointclouds) * 3))):
