...
class RayTracer:
    def __init__(self) -> None:
        ...
        mi.set_variant('cuda_ad_rgb')
        self.angle = 1.0
        self.scene = mi.load_dict(get_deafult_scene(idx=0, angle=self.angle))
...
def get_deafult_scene(idx, angle):
    ...
    default_scene ={
            ...
            'smpl':{
                ...
                'to_world' : T.translate([0,0,0]) @ T.rotate([0,1,0], (idx*angle)-90) @ T.rotate([0,0,1], 180) @ T.rotate([1,0,0], 90) @ T.scale([1, 1, 1]),
                "mybsdf": {
                    "type": "ref",
                    "id": "while"
                },
